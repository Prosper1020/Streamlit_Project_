...
if page == pages[3]:
    ...
    if task_type == "Regression":
        ...
        if features and target:
            ...
            # Model training and saving to a file
            def train_and_save_model(model, model_file):
            ...
            if fuel_option == "With Fuel Consumption":
                    LR = LinearRegression()
                    train_and_save_model(LR, 'LR_model_with_fuel.pkl')
                    Lasso = LassoCV()
                    train_and_save_model(Lasso, 'Lasso_model_with_fuel.pkl')
                    Ridge = RidgeCV()
                    train_and_save_model(Ridge, 'Ridge_model_with_fuel.pkl')
            else:
                    LR = LinearRegression()
                    train_and_save_model(LR, 'LR_model_without_fuel.pkl')
                    Lasso = LassoCV()
                    train_and_save_model(Lasso, 'Lasso_model_without_fuel.pkl')
                    Ridge = RidgeCV()
                    train_and_save_model(Ridge, 'Ridge_model_without_fuel.pkl')
    ...
    # Classification Model Selection
        ...
        if features and target:
            ...
            if fuel_option == "With Fuel Consumption":
                    ...
                    train_and_save_model(random_forest_model, 'random_forest_model_with_fuel.pkl')
                    decision_tree_model = DecisionTreeClassifier()
                    train_and_save_model(decision_tree_model, 'decision_tree_model_with_fuel.pkl')
            else:
                    random_forest_model = RandomForestClassifier()
                    train_and_save_model(random_forest_model, 'random_forest_model_without_fuel.pkl')
                    decision_tree_model = DecisionTreeClassifier()
                    train_and_save_model(decision_tree_model, 'decision_tree_model_without_fuel.pkl')
